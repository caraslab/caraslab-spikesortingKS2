function [data, timestamps, info, is_eof] = load_open_ephys_data_chunked(filename,t1, t2, chunk_datatype)
%
...
%     disp(['Loading ' filename '...']);
    ...
    % MML edit: optionally change the type of the range given from seconds
    % to samples
    if strcmp(chunk_datatype, 'time')
        blockt1 = (t1*Fs)/SAMPLES_PER_RECORD;
        blockt2 = (t2*Fs)/SAMPLES_PER_RECORD;
    elseif strcmp(chunk_datatype, 'samples')
        blockt1 = t1/SAMPLES_PER_RECORD;
        blockt2 = t2/SAMPLES_PER_RECORD;
    else
        display('Invalid range format (chunk_datatype)')
        return
    end
